...
export default function SettingsDialog() {
  ...
  const { config, setConfig, connected, language } = useLiveAPIContext();
  ...
  const systemInstruction = useMemo(() => {
    ...
    if (!config.systemInstruction) {
    ...
    } else if (Array.isArray(config.systemInstruction)) {
      baseInstruction = config.systemInstruction
      ...
      baseInstruction = config.systemInstruction.parts?.map((p) => p.text).join("\n") || "";
    }
    
    // Use utility function to build system prompt with language instruction
    return buildSystemPrompt(baseInstruction, language);
  }, [config, language]);
